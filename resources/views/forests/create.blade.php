...
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">{{ __('forest.create') }}</div>
            <form method="POST" action="{{ route('forests.store') }}" accept-charset="UTF-8">
                {{ csrf_field() }}
                <div class="card-body">
                ...
                <div class="card-footer">
                    <input type="submit" value="{{ __('forest.create') }}" class="btn btn-success">
                    <a href="{{ route('forests.index') }}" class="btn btn-link">{{ __('app.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
